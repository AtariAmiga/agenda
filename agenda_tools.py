...
def draw_header(canvas, x, date, gs):
    ...
    canvas.drawString(x + gs.margin, gs.page_height - font_ascent(canvas) - gs.margin, num_day)

    day_name = date.strftime('%A').upper()
    canvas.setFont(gs.font_name, 20)
    a = font_ascent(canvas)
    canvas.drawRightString(x + gs.col_width() - gs.margin, gs.page_height - gs.margin - a, day_name)

    month_and_year = date.strftime('%B %Y')
    canvas.setFont(gs.font_name, 12)
    canvas.drawRightString(x + col_width - gs.margin, gs.page_height - gs.margin - 2 * a, month_and_year)
